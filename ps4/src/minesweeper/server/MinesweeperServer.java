...
public class MinesweeperServer {
    ...
    private ConcurrentLinkedQueue<BoardMessage> boardMessages;  
    private BoardProcess mBoardProcess;
    ...
    private void startBoardProcess(Optional<File> file, int sizeX, int sizeY) throws IOException {
    			Thread boardProcessThread;
    			// Board process thread
    			boardMessages = new ConcurrentLinkedQueue<>();
    			mBoardProcess = new BoardProcess(boardMessages, sizeX, sizeY, file);
    			boardProcessThread = new Thread(mBoardProcess);
    ...
    private String handleRequest(String input) {
    		...
    		final Condition processFinish = msgLock.newCondition();
        ...
        if (tokens[0].equals("look")) {
            // 'look' request
            return mBoardProcess.getmBoard().showBoard();
            ...
            BoardMessage message = new BoardMessage(msgLock, processFinish);
            String response;
            ...
            if (tokens[0].equals("dig")) {
                // 'dig x y' request
            		message.setMsgType(MsgType.DIG);
            } else if (tokens[0].equals("flag")) {
                // 'flag x y' request
            		message.setMsgType(MsgType.FLAG);
            } else if (tokens[0].equals("deflag")) {
                // 'deflag x y' request
            		message.setMsgType(MsgType.DEFLAG);
            }
            boardMessages.add(message);
	    		msgLock.lock();
	    		try {
	    			processFinish.awaitUninterruptibly();
    ...
    public static void runMinesweeperServer(boolean debug, Optional<File> file, int sizeX, int sizeY, int port) throws IOException {
        ...
        server.startBoardProcess(file, sizeX, sizeY);
        server.serve();
    }
}
